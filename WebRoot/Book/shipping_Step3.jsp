...
<%
//保存当前页面信息
   session.setAttribute("THIS", "Book/shipping_Step3.jsp");
									...
									<td height="225" align="center">
										<table width="845" height="69" border="0" align="center">
											<tr>
												<td colspan="7">
													<center>
			<form name="send_Order" action="../do/doZhifu.jsp" method="post" onsubmit="return tijiao()">
...
List<CartsInfo> list2=cart.getCartInfo(user.getuId());
 %>
											<tr align="center">
...
for(int i=0;i<list2.size();i++){
 CartsInfo info2=list2.get(i);
 num=num+Integer.parseInt(info2.getgCount()); %>
											<tr align="center">
												<td><%=info2.getCartId() %></td>
												<td><%=info2.getgName() %></td>
												<td><%=info2.getgCount() %></td>
												<td><%=Double.parseDouble(info2.getgPrice()) %></td>
												<td><%=Double.parseDouble(info2.getgPrice())*Integer.parseInt(info2.getgCount()) %></td>
												<td align="left"><%=info2.getcBeizhu() %></td>												
											</tr>
											<%}} %>
											<tr>
												<td colspan="2">&nbsp;</td>
												<td class="red" align="center"><font size="3" class="red">共：<%=num %>份</font></td>
												
												<td colspan="2">
													<font size="3" class="red">
														总计：<%=cart.sum(user.getuId()) %>元
													</font></td>
												<td align="left">&nbsp;</td>
											</tr>
										</table></td>
