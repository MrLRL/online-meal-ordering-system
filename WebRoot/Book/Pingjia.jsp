...
<%
    ...
    String leixing=request.getParameter("leixing");//获得类型
...
function xuanzhong() {
	...
	leixing[<%=Integer.parseInt(leixing)-1 %>].checked=true;
}
</script>
</head>
<%
  ...
  int pIndex=1;//当前页数
	...
	//调用查询方法
  List<PingjiasInfo> list=dao.getleixing(pIndex, leixing, gId);
  %>
<body>
		<jsp:include page="../includeFile/shippingTop.jsp"></jsp:include>
		<table align="center"><tr><td align="center" width="550px" id="chakan">
		<a href="Pinglun.jsp?gId=<%=gId %>">查看所有评论</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<script type="text/javascript">xuanzhong();</script>
...
<%
     int sum=dao.num1(leixing, gId);//获得总条数
     int n=0;//总页数
     if(sum%5==0){
     n=sum/5;
     }else{
...
<%
 ...
 %>
<div id="index" align="center">首页&nbsp; 上一页&nbsp; <a href="Pingjia.jsp?leixing=<%=leixing %>&gId=<%=gId %>&pages=<%=pIndex+1 %>">下一页</a>&nbsp; <a href="Pingjia.jsp?leixing=<%=leixing %>&gId=<%=gId %>&pages=<%=n%>">末页</a>&nbsp;&nbsp;&nbsp;</div>

<%
} else if(pIndex==n){
%>
<div id="index" align="center"><a href="Pingjia.jsp?leixing=<%=leixing %>&gId=<%=gId %>&pages=1">首页</a>&nbsp; <a href="Pingjia.jsp?leixing=<%=leixing %>&gId=<%=gId %>&pages=<%=pIndex-1%>">上一页</a>&nbsp; 下一页&nbsp; 末页&nbsp;&nbsp;&nbsp;</div>
<%
} else{
%>
<div id="index" align="center"><a href="Pingjia.jsp?leixing=<%=leixing %>&gId=<%=gId %>&pages=1">首页</a>&nbsp; <a href="Pingjia.jsp?leixing=<%=leixing %>&gId=<%=gId %>&pages=<%=pIndex-1%>">上一页</a>&nbsp; <a href="Pingjia.jsp?leixing=<%=leixing %>&gId=<%=gId %>&pages=<%=pIndex+1%>">下一页</a>&nbsp; <a href="Pingjia.jsp?leixing=<%=leixing %>&gId=<%=gId %>&pages=<%=n%>">末页</a>&nbsp;&nbsp;&nbsp;</div>
<%
}
%>
